...
struct YouTrackAPIConfiguration: Sendable {
    ...
    // Idle timeout between received packets, not a whole-transfer limit. 120s made
    // interactive fetches linger for two minutes on a dead connection.
...
struct YouTrackAPIClient: Sendable {
    ...
    func get(path: String, queryItems: [URLQueryItem]) async throws -> Data {
        ...
        let request = try await makeRequest(method: "GET", url: url)
        return try await execute(request)
    }

    func post(path: String, queryItems: [URLQueryItem] = [], body: Data?) async throws -> Data {
        try await AppDebugSettings.applySlowResponseIfNeeded()
        let url = try makeURL(path: path, queryItems: queryItems.isEmpty ? nil : queryItems)
        var request = try await makeRequest(method: "POST", url: url)
    ...
    func delete(path: String, queryItems: [URLQueryItem] = []) async throws {
        try await AppDebugSettings.applySlowResponseIfNeeded()
        let url = try makeURL(path: path, queryItems: queryItems.isEmpty ? nil : queryItems)
        let request = try await makeRequest(method: "DELETE", url: url)
...
private extension YouTrackAPIClient {
    ...
    func makeRequest(method: String, url: URL) async throws -> URLRequest {
